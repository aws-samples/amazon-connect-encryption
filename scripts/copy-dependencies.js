...
const processDependencyFile = async (file) => {
	...
	file = file.replace(/\\/g, '/');
	console.log(`processDependencyFile: file ${file}`);

	// set the target directory
	const targetDir = 'bin/'.concat(file).replace(dependencyFileName, '');
	console.log(`processDependencyFile: targetDir ${targetDir}`);

	const targetModulesDir = `${targetDir}`.concat(nodeDirInLambda, '/', nodeModulesDir);
	console.log(`processDependencyFile: targetModulesDir ${targetModulesDir}`);

	// delete directory recursively
	if (fs.existsSync(targetModulesDir)) {
		console.log(`processDependencyFile: Deleting ${targetModulesDir}`);
	...
	for (let dependency of dependencies) {
		...
		if (!dependency == '') {
			copyPromises.push(findModuleDependencies(nodeModulesDir, dependency, targetModulesDir));
		}
	}
	...
	for (let pkg of uniquePackages) {
		const source = nodeModulesDir.concat('/', pkg);
		const target = targetModulesDir.concat('/', pkg);
		//console.log(`copyDependencies: Copying [${source}] to [${target}]`);
		copyPromiseArray.push(fsExtra.copy(source, target));
	}
...
const processLayerFile = async (oldPath) => {
	...
	fs.rename(oldPath, newPath, (async err => {
		...
		if(fs.existsSync(oldLayerDir)) {
			...
			console.log(`processLayerFile: [${oldLayerDir}] has ${files.length} files`, files);
			if(files.length < 1) {
...
const getAllFiles = function (dirPath, arrayOfFiles) {
	...
	files.forEach(function (file) {
		const fName = `${dirPath}/${file}`;
		console.log(`getAllFiles: Processing [${fName}]`);
		if (fs.statSync(fName).isDirectory()) {
			arrayOfFiles = getAllFiles(fName, arrayOfFiles);
		} else {			
			arrayOfFiles.push(fName);
		}
	});
...
const getAllLayerFiles = (dirPath, layerFiles = []) => {
	...
	if(dirPath.startsWith('bin/test') || dirPath.endsWith(nodeDirInLambda) || dirPath.endsWith(nodeModulesDir)) {
		return layerFiles;
	}
	...
	let files = fs.readdirSync(dirPath);
	console.log(`getAllLayerFiles:`, files);
...
const copyDependencies = async (startDir) => {
	...
	console.log(`copyDependencies: Checking for dependencies in [${startDir}] ...`);
...
if (require.main === module) {
	copyDependencies(process.argv[2] || './src');
}
